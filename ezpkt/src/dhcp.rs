...
impl Dhcp {
    ...
    pub fn ciaddr(self, ciaddr: u32) -> Self {
        self.dhcp.mutate(&self.pkt, |hdr| hdr.ciaddr = ciaddr.to_be());
        self
    }

    #[must_use]
    pub fn yiaddr(self, yiaddr: u32) -> Self {
        self.dhcp.mutate(&self.pkt, |hdr| hdr.yiaddr = yiaddr.to_be());
        self
    }

    #[must_use]
    pub fn siaddr(self, siaddr: u32) -> Self {
        self.dhcp.mutate(&self.pkt, |hdr| hdr.siaddr = siaddr.to_be());
        self
    }

    #[must_use]
    pub fn giaddr(self, giaddr: u32) -> Self {
        self.dhcp.mutate(&self.pkt, |hdr| hdr.giaddr = giaddr.to_be());
        self
    }

    #[must_use]
    pub fn chaddr<T: AsRef<[u8]>>(self, chaddr: T) -> Self {
        self.dhcp.mutate(&self.pkt, |hdr| { hdr.set_chaddr(chaddr); });
    ...
    pub fn sname<T: AsRef<[u8]>>(self, sname: T) -> Self {
        self.dhcp.mutate(&self.pkt, |hdr| { hdr.set_sname(sname); });
    ...
    pub fn file<T: AsRef<[u8]>>(self, file: T) -> Self {
        self.dhcp.mutate(&self.pkt, |hdr| { hdr.set_file(file); });
